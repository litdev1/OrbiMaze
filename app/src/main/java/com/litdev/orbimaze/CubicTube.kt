...
class CubicTube(pos1: Vector3, pos2: Vector3, dir1: Vector3, dir2:Vector3, radius: Float, sides: Int, segments: Int) {
    val pos1 = pos1
    val pos2 = pos2
    val dir1 = dir1
    val dir2 = dir2
    ...
    fun build() {
        val A = pos1
        val B = dir1
        val C = Vector3(-2*dir1.x -dir2.x+3*(pos2.x-pos1.x),
            -2*dir1.y -dir2.y+3*(pos2.y-pos1.y),
            -2*dir1.z -dir2.z+3*(pos2.z-pos1.z))
        val D = Vector3(dir1.x+dir2.x-2*(pos2.x-pos1.x),
        ...
        for (i in 0..segments) {
            val r = i.toFloat() / segments //parametric distance along the curve
            val pos = Vector3(
            ...
            for (j in 0..sides - 1) {
                ...
                val radDir = Quaternion.rotateVector(rot, rad).normalized() //vector in radial direction after rotation
                val x = pos.x + radius*radDir.x
                val y = pos.y + radius*radDir.y
                val z = pos.z + radius*radDir.z
        ...
        boundingBox.centerPosition = Float3((min.x + max.x) / 2.0f, (min.y + max.y) / 2.0f, (min.z + max.z) / 2.0f)
        boundingBox.halfExtentSize = Float3((max.x - min.x) / 2.0f, (max.y - min.y) / 2.0f, (max.z - min.z) / 2.0f)
    }
